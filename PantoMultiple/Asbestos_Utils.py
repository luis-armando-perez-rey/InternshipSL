...
def load_image(img_path, img_name):
    ...
    img = cv2.imread(join(img_path, img_name))  # read as gray scale (type: uint8)
...
def generate_complete_flipping(load_path, save_path, data_type='.jpg', x_flip=True, y_flip=True,
    ...
    for i in range(int(len(names_images) * percentage_data_generation)):
        ...
        image = cv2.imread(join(load_path,names_images[i]))
        if isfile(join(load_path, names_images[i].replace(data_type, '_FIB' + data_type))):
            annotation = cv2.imread(join(load_path, names_images[i].replace(data_type, '_FIB' + data_type)),0)
            annotation2 = cv2.imread(join(load_path, names_images[i].replace(data_type, '_FIB2' + data_type)),0)
        else:
        ...
        for method in methods:
            ...
            cv2.imwrite(join(save_path, names_images[i].replace(data_type, '_XDA' + str(method) + '_FIB2' + data_type)),
                        image_flipping(annotation2, method))
