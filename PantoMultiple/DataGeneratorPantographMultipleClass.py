...
class DataGeneratorPantographMultipleClass(object):
    ...
    def __data_generation(self, batch_names):
        ...
        y = np.empty((self.batch_size, self.size[0], self.size[1], 2))
        for i, name in enumerate(batch_names):
            ...
            if isfile(join(self.path, name.replace(self.data_type, '_FIB' + self.data_type))):
                ...
                label2 = cv2.imread(join(self.path, name.replace(self.data_type,'_FIB2' + self.data_type)),0)

                _, label2 = cv2.threshold(label2, 127, 255, 0)
            ...
            y[i, :, :, :] = 1 * (label == 255) # Classify as 1 if the value is 255 for the "fiber" mask.
