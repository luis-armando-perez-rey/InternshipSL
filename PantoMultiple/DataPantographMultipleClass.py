...
class DataPantographMultipleClass(object):
    """
    ...
    def get_chip_images(self,size):
        ...
        if not(exists(folder_array[0])):
            ...
            for name in names:
                ...
                label2 = cv2.imread(join(self.path_name,name.replace(get_file_extension(name),'_FIB2'+get_file_extension(name))),0)
                label2 = label2[:,:,np.newaxis]
                label = np.concatenate((label,label2),axis = 2)
                self.convolutional_selection(folder_array[0], size, image, label, name)
            if self.augmentation:
                generate_complete_flipping(folder_array[0],folder_array[0], data_type=self.load_extension,x_flip= False,xy_flip=False)
        ...
        for num_folder in range(2):
            ...
            for name in names:
                ...
                cv2.imwrite(join(folder,
                                 name.replace(get_file_extension(name),
                                              '_FIB2' + get_file_extension(name))), label2)
